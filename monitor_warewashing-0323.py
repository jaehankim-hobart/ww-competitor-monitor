...
def get_user_token():
    ...
    device_code_url = f"https://login.microsoftonline.com/{GRAPH_TENANT}/oauth2/v2.0/devicecode"
    data = {
        "client_id": GRAPH_CLIENT_ID,
        "scope": "https://graph.microsoft.com/Mail.Send"
    }
    dbg("[GRAPH] Requesting device code…")
    r = requests.post(device_code_url, data=data, timeout=25)
    r.raise_for_status()
    device_info = r.json()
    
    print("Please go to:", device_info['verification_uri'])
    print("And enter the code:", device_info['user_code'])
    ...
    while True:
        ...
        if error == 'authorization_pending':
            ...
            time.sleep(device_info['interval'] + 5)
            continue
        else:
            raise Exception(f"Authentication failed: {r.json()}")
